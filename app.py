...
ZENDS_PRODUCTS = {
    ...
    "Digital Services": {
        "ZENDMeet": {
            "price": "USA: $150, India: $90, Singapore: $165, Thailand: $120",
            "features": ["Video Conferencing", "Screen Sharing", "Recording", "1TB Cloud Storage", "Up to 100 participants"],
            "enterprise_price": "USA: $130, India: $78, Singapore: $143, Thailand: $104"
        },
        "ZENDStream": {
            "price": "USA: $120, India: $72, Singapore: $132, Thailand: $96",
            "features": ["Live Streaming", "Content Delivery", "Analytics", "Multi-platform Support", "HD Quality"],
            "enterprise_price": "USA: $100, India: $60, Singapore: $110, Thailand: $80"
        }
    }
}
...
def generate_smart_product_response(query: str, products: List[Dict], policies: List[Dict]) -> str:
    """Generate intelligent response based on specific features/questions in query"""
    query_lower = query.lower()
    
    # Extract what the user is asking about
    asking_about = {
        'fup_limit': any(word in query_lower for word in ['fup', 'fair usage', 'usage limit', '1tb limit']),
        'sla': any(word in query_lower for word in ['sla', 'uptime', '99.9%', '99.5%', '98.5%']),
        'predictive_maintenance': any(word in query_lower for word in ['predictive maintenance', 'predictional maintenance']),
        '5g_data': any(word in query_lower for word in ['5g', '5g data']),
        'consolidated_invoicing': any(word in query_lower for word in ['consolidated invoice', 'consolidation invoicing', 'consolidated billing', 'consolidation billing']),
        'support': 'support' in query_lower or 'does' in query_lower or 'supports' in query_lower,
        'details': 'details' in query_lower or 'detail' in query_lower or 'information' in query_lower
    }
    ...
    if products and len(products) > 0:
        ...
        product_features = product['info']['features']
        product_price = product['info']['price']
        product_category = product['category']
        
        response_parts = []
        response_parts.append(f"Thank you for asking about **{product_name}**!\n")
        
        # Answer specific questions
        if asking_about['fup_limit']:
            # Check if product has unlimited data
            has_unlimited = any('unlimited' in f.lower() for f in product_features)
            if has_unlimited:
                response_parts.append(f"**📊 Fair Usage Policy (FUP):**\n")
                response_parts.append(f"Yes, {product_name} includes unlimited data with a Fair Usage Policy (FUP) limit of **1TB per month**. After reaching 1TB, speeds may be throttled during peak hours. The FUP resets every billing cycle.\n")
            else:
                response_parts.append(f"**📊 Data Limits:**\n")
                data_features = [f for f in product_features if 'gb' in f.lower() or 'data' in f.lower()]
                if data_features:
                    response_parts.append(f"{product_name} comes with {', '.join(data_features)}. There is no separate FUP limit as the plan has a fixed data allocation.\n")
        
        if asking_about['sla']:
            # SLA information based on customer type
            response_parts.append(f"**⏱️ Service Level Agreement (SLA):**\n")
            if 'enterprise' in product_name.lower() or 'business' in product_category.lower():
                response_parts.append(f"Yes! {product_name} includes an **SLA guarantee of 99.9% uptime** for enterprise customers. This means maximum downtime of ~43 minutes per month. SLA credits are provided for any downtime exceeding this guarantee.\n")
            elif 'office' in product_name.lower() or 'biz' in product_name.lower():
                response_parts.append(f"Yes! {product_name} includes an **SLA guarantee of 99.5% uptime** for business customers. Compensation is provided if uptime falls below this threshold.\n")
            else:
                response_parts.append(f"{product_name} comes with a **98.5% uptime guarantee** for individual users. Enterprise plans offer higher SLA tiers (99.5% - 99.9%).\n")
        
        if asking_about['predictive_maintenance']:
            # Check if it's an IoT or industrial product
            if 'iot' in product_category.lower() or 'industrial' in product_name.lower() or 'smart' in product_name.lower():
                response_parts.append(f"**🔧 Predictive Maintenance:**\n")
                response_parts.append(f"Yes! {product_name} includes **Predictive Maintenance** capabilities. The system uses real-time data and analytics to predict potential issues before they occur, minimizing downtime and optimizing performance.\n")
            else:
                response_parts.append(f"**🔧 Maintenance Support:**\n")
                response_parts.append(f"{product_name} includes 24/7 technical support and proactive monitoring, though predictive maintenance is primarily available for our IoT and industrial solutions.\n")
        
        if asking_about['5g_data']:
            # Check if it's a mobile product
            if 'mobile' in product_category.lower() or 'prepaid' in product_name.lower() or 'postpaid' in product_name.lower():
                response_parts.append(f"**📡 5G Data Support:**\n")
                response_parts.append(f"Yes! {product_name} supports **5G data connectivity** where available. You'll get ultra-fast speeds, low latency, and enhanced capacity for streaming, gaming, and downloads.\n")
            else:
                response_parts.append(f"**📡 Network Support:**\n")
                response_parts.append(f"{product_name} operates on our high-speed network infrastructure. For 5G mobile data, please check our mobile connectivity plans (Prepaid/Postpaid).\n")
        
        if asking_about['consolidated_invoicing']:
            # Enterprise feature
            response_parts.append(f"**📄 Consolidated Invoicing:**\n")
            if 'enterprise' in product_name.lower():
                response_parts.append(f"Yes! {product_name} includes **consolidated invoicing** for enterprise customers. All your services are combined into a single monthly invoice for easier accounting and management.\n")
            else:
                response_parts.append(f"Consolidated invoicing is available as an enterprise feature. Individual plans receive separate invoices, but you can upgrade to enterprise plans to access consolidated billing.\n")
        
        # Always show product details at the end
        response_parts.append(f"\n**📦 Product Details:**\n")
        response_parts.append(f"- **Category**: {product_category}\n")
        response_parts.append(f"- **Pricing**: {product_price}\n")
        response_parts.append(f"- **Key Features**: {', '.join(product_features)}\n")
        
        response_parts.append(f"\n**Need more information?** Contact our sales team at sales@zends.com or call 1-800-ZENDS-SALES.")
        
        return "\n".join(response_parts)
    
    # Fallback if no product found
    return "I couldn't find specific information about that product. Please check the product name and try again, or contact our support team for assistance."
...
def detect_intent_ml(query: str) -> Tuple[str, float]:
    ...
    # CRITICAL OVERRIDES FOR CSV DATA PATTERNS
    if any(k in query_lower for k in ['refund', 'return my money', 'money back', 'return was for']):
        return 'refund', 0.98
    
    if any(k in query_lower for k in ['bill', 'billing', 'charged', 'invoice', 'payment']):
        if 'refund' not in query_lower:  # Don't override if it's about billing refund
            return 'billing', 0.98
    
    if any(k in query_lower for k in ['not working', 'slow', 'down', 'outage', 'is back']):
        if 'bill' not in query_lower and 'refund' not in query_lower:
            return 'technical', 0.98
    
    if any(k in query_lower for k in ['complaint', 'complain', 'dissatisfied', 'terrible', 'awful', 'unacceptable']):
        return 'complaint', 0.98
    
    # Check for product inquiry patterns
    product_keywords = ['zendcloud', 'zendfiber', 'zendbiz', 'zendsmart', 'zendmeet', 'zendstream',
                       'zendindustrial', 'prepaid', 'postpaid', 'vm basic', 'vm pro', 
                       'platinum', 'gold', 'silver', 'basic', 'plus', 'unlimited']
    
    if any(pk in query_lower for pk in product_keywords):
        inquiry_words = ['tell me', 'what is', 'information', 'details', 'features', 'price', 
                        'support', 'does', 'about', 'detail of']
        problem_words = ['not working', 'issue', 'problem', 'slow', 'down', 'charged']
        
        has_inquiry = any(iw in query_lower for iw in inquiry_words)
        has_problem = any(pw in query_lower for pw in problem_words)
        
        if has_inquiry and not has_problem:
            return 'product', 0.95
...
def detect_intent_rules(query: str) -> Tuple[str, float]:
    ...
    # MAJOR FIX: Enhanced product name detection including CSV products
    product_names = [
        ...
        'zendmeet', 'zendstream', 'basic', 'plus', 'unlimited'
    ]
    
    has_product_name = any(pname in query_lower for pname in product_names)
    
    # Enhanced product inquiry phrases from CSV patterns
    product_inquiry_phrases = [
        'tell me about', 'what is', 'information about', 'details about', 'details of', 'detail of',
        'specs', 'specification', 'features of', 'price of', 'cost of',
        'how much', 'pricing for', 'tell me more', 'about',
        'support', 'does', 'supports'
    ]
...
def retrieve_relevant_products(query: str, top_k: int = 3) -> List[Dict]:
    """
    FIXED: Retrieve products with EXACT name matching priority
    """
    query_lower = query.lower()
    
    # Direct exact matching - highest priority
    matched_products = []
    ...
    for category, products in ZENDS_PRODUCTS.items():
        for product_name, product_info in products.items():
            ...
            # PRIORITY 1: Exact full product name in query (e.g., "zendcloud vm pro" in query)
            if product_name_lower in query_lower:
                score = 1.0
                
            # PRIORITY 2: Check if ALL key words from product name are in query
            else:
                # Extract significant words (exclude common words)
                product_words = [w for w in product_name_lower.split() if len(w) > 2]
                
                # Count how many product words appear in query
                matches = sum(1 for w in product_words if w in query_lower)
                
                if len(product_words) > 0:
                    word_match_ratio = matches / len(product_words)
                    
                    # Only consider it a match if most words match
                    if word_match_ratio >= 0.8:  # 80% of words must match
                        score = word_match_ratio * 0.9
                    elif word_match_ratio >= 0.6:  # 60-79% match gets lower score
    ...
    # Return top matches
    return matched_products[:top_k] if matched_products else []
...
def generate_ai_response(query: str, intent: str, sentiment: str, 
    ...
    """
    FIXED: Generate AI response with special handling for feature queries
    """
    query_lower = query.lower()
    
    # Handle complaints first
    if is_complaint:
...
ZENDS Customer Support"""
    ...
    # CRITICAL: Check for CONSOLIDATED INVOICING questions
    consolidated_keywords = ['consolidated invoice', 'consolidated invoicing', 'consolidated billing', 
                            'consolidation invoice', 'consolidation billing']
    
    asking_about_consolidated = any(keyword in query_lower for keyword in consolidated_keywords)
    
    if asking_about_consolidated and intent == 'product':
        # Get product name
        product_name = "this product"
        if products and len(products) > 0:
            product_name = products[0]['name']
        
        return f"""**Yes!** {product_name} supports **Consolidated Invoicing** for enterprise customers.

**📄 Consolidated Invoicing (from ZENDS Policy):**
According to ZENDS Communications policy: **"Enterprise customers receive consolidated invoices."**

This means:
✅ All your ZENDS services are combined into a single monthly invoice
✅ Simplified billing management and accounting  
✅ Net-30 payment terms available for enterprise customers
✅ Volume discounts up to 30% for bulk enterprise users
✅ Quarterly or annual billing cycles supported

**Billing Policy Details:**
• Monthly billing in advance
• Enterprise customers receive consolidated invoices
• Late payment after 7 days may suspend services
• Payment processing within 15 days of invoice generation

**To Enable Consolidated Invoicing:**
1. Upgrade to enterprise pricing tier for {product_name}
2. Contact enterprise sales: sales@zends.com
3. Call: 1-800-ZENDS-SALES

**Note:** Individual plan customers receive separate invoices for each service."""
    
    # Check for FUP questions
    fup_keywords = ['fup', 'fair usage', 'usage limit', '1tb limit']
    asking_about_fup = any(keyword in query_lower for keyword in fup_keywords)
    
    if asking_about_fup and intent == 'product':
        product_name = "this product"
        if products and len(products) > 0:
            product_name = products[0]['name']
            # Check if it's an unlimited plan
            features = products[0]['info'].get('features', [])
            has_unlimited = any('unlimited' in f.lower() for f in features)
            
            if has_unlimited:
                return f"""**Yes!** {product_name} includes unlimited data with a **Fair Usage Policy (FUP)**.

**📊 Fair Usage Policy (from ZENDS Policy):**
According to ZENDS Communications policy: **"Unlimited plans are capped at 1TB per month."**

**FUP Details:**
• **FUP Limit:** 1TB (1,000 GB) per month
• **After FUP:** Speeds may be throttled during peak hours
• **Reset:** FUP resets every billing cycle
• **Enterprise Plans:** Custom FUP based on agreement

This ensures fair network usage while providing unlimited data for most users."""
    
    # Check for SLA questions
    sla_keywords = ['sla', 'uptime', '99.9', '99.5', '98.5']
    asking_about_sla = any(keyword in query_lower for keyword in sla_keywords)
    
    if asking_about_sla and intent == 'product':
        product_name = "this product"
        if products and len(products) > 0:
            product_name = products[0]['name']
            pname_lower = product_name.lower()
            
            # Determine SLA based on product type
            if 'enterprise' in pname_lower:
                sla = "99.9%"
                customer_type = "Enterprise"
            elif any(word in pname_lower for word in ['business', 'office', 'biz']):
                sla = "99.5%"
            ...
            return f"""**Yes!** {product_name} includes an **SLA guarantee**.

**⏱️ SLA Policy (from ZENDS Policy):**
According to ZENDS Communications policy:
• **Individual users:** 98.5% uptime
• **Business users:** 99.5% uptime  
• **Enterprise users:** 99.9% uptime

**For {product_name} ({customer_type} tier):**
• **Uptime Guarantee:** {sla}
• **Compensation:** SLA credits for downtime exceeding guarantee
• **Support:** 24/7 technical support
• **Monitoring:** Proactive network monitoring"""
    
    # Check for Predictive Maintenance questions
    pm_keywords = ['predictive maintenance', 'predictional maintenance']
    asking_about_pm = any(keyword in query_lower for keyword in pm_keywords)
    
    if asking_about_pm and intent == 'product':
        product_name = "this product"
        if products and len(products) > 0:
            product_name = products[0]['name']
            pname_lower = product_name.lower()
            
            # Only IoT/Industrial products have predictive maintenance
            if any(word in pname_lower for word in ['industrial', 'sensor', 'iot', 'fleet', 'smart']):
                return f"""**Yes!** {product_name} includes **Predictive Maintenance** capabilities.

**🔧 Predictive Maintenance Features:**
• Real-time sensor data collection and analysis
• AI-powered prediction of potential issues before they occur
• Minimizes downtime through early detection
• Analytics dashboard with maintenance insights
• Alerts and notifications for predicted issues

This helps you optimize performance and reduce unexpected failures."""
            else:
                return f"""**Predictive Maintenance** is available for our IoT and Industrial solutions.

{product_name} includes:
• 24/7 technical support
• Proactive monitoring
• Remote diagnostics

For Predictive Maintenance capabilities, please check:
• ZENDIndustrial Sensor
• ZENDFleet IoT
• Other IoT & Smart Solutions"""
    
    # Default response for product queries
    if intent == 'product' and products and len(products) > 0:
        product = products[0]
        product_text = f"""Thank you for your inquiry about **{product['name']}**!

**📦 Product Details:**
• **Category:** {product['category']}
• **Pricing:** {product['info']['price']}
• **Enterprise Pricing:** {product['info'].get('enterprise_price', 'Contact sales')}
• **Key Features:** {', '.join(product['info']['features'])}
...
Submit your request at: refunds@zends.com with your account details.""",
        ...
        'technical': """I understand you're experiencing technical issues.
...
Still having issues? Our technical team is ready to help immediately.""",
        ...
        'others': """Thank you for your inquiry.
...
def process_query(query: str) -> Dict:
    ...
    try:
        ...
        if is_out_of_scope(query):
            ...
            st.session_state.edited_response = result['response']
            return result
        ...
        # Apply keyword overrides for better CSV accuracy
        if any(k in query_lower for k in ['refund', 'money back', 'return my money', 'return was for']):
            intent = 'refund'
            confidence = 0.98
        elif any(k in query_lower for k in ['bill', 'billing', 'charged', 'invoice']):
            if 'refund' not in query_lower:
                intent = 'billing'
                confidence = 0.98
        elif any(k in query_lower for k in ['not working', 'slow', 'down', 'outage', 'is back']):
            if 'bill' not in query_lower and 'refund' not in query_lower:
                intent = 'technical'
                confidence = 0.98
        ...
        # If strong product match found, prioritize it
        if products and len(products) > 0 and products[0].get('similarity', 0) > 0.55:
            if intent not in ['complaint', 'refund', 'billing', 'technical']:
                intent = 'product'
